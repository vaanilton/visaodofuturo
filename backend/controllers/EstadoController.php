...
class EstadoController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        $modelsEstado=(new \yii\db\Query())
        ->select(['id','p.user_iduser as user', 'p.photo','p.nome','data_hr_inicio', 'data_hr_fim'])
        ->from('Estado es')
        ->join('join','profile p','p.user_iduser = es.user_iduser');

        $pages = new Pagination(['totalCount' => $modelsEstado->count(), 'pageSize'=>10]);
        $modelsEstado = $modelsEstado->offset($pages->offset)
            ->limit($pages->limit)
            ->all();

        return $this->render('index', [
            'modelsEstado' => $modelsEstado,
